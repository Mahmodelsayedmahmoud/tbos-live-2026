...
export function login(username: string, password: string): { success: boolean; user?: User; error?: string } {
  if (!username || !password) return { success: false, error: 'missing_credentials' };
...
export function bulkImportCouriers(couriersData: Array<Omit<Courier, 'id' | 'status'>>): { success: number; failed: number; errors: string[] } {
  ...
  couriersData.forEach((data, index) => {
    try {
      ...
      if (!branchId && defaultBranch) branchId = defaultBranch.id;
      if (branchId) {
        const branch = state.branches.find(b => b.id === branchId || b.name === branchId || b.code === branchId);
        if (!branch && defaultBranch) branchId = defaultBranch.id;
        else if (branch) branchId = branch.id;
      }
...
export function updateTripStatus(id: string, status: TripStatus): Trip | null {
  ...
  if (status === 'COMPLETED') state.trips[idx].completedAt = new Date().toISOString();
  saveState(state);
...
export function startStage(tripId: string, stage: StageName): { success: boolean; error?: string } {
  ...
  if (activeStages.length === 1) trip.currentStage = stage;
...
export function finishStage(tripId: string, stage: StageName): { success: boolean; error?: string } {
  ...
  if (activeStages.length > 0) {
    const sortedActive = activeStages.sort((a, b) => STAGE_ORDER.indexOf(a.stage) - STAGE_ORDER.indexOf(b.stage));
    ...
    if (pendingStages.length > 0) {
      const sortedPending = pendingStages.sort((a, b) => STAGE_ORDER.indexOf(a.stage) - STAGE_ORDER.indexOf(b.stage));
...
export function addToQueue(tripId: string, branchId: string, priority: number = 5): QueueRecord | null {
  ...
  const maxQueueNumber = branchQueue.length > 0 ? Math.max(...branchQueue.map(q => q.queueNumber)) : 0;
